...
Name generate_random(){
  Name *res = NULL;
  res = (Name*)malloc(sizeof(Name)); //allocate memory on heap for the Name.

  int gender = rand() % 2;
  if(gender == 0){              //when gender equals 0, it the name will be chosen from female_names[], when it equals 1, from male_names[]
    res->first = female_names[rand() % LENGTH]; // produce a random index in [0,9] inclusively.
    int choose = rand() % 2;    //decide if middle name is null
    if(choose == 0)
      res->middle = NULL;
    else
 ...
 */
void add_some_kids(Couple *couple, int n){
  generate_random_array(couple->kids, n);
}
...
Couple wedding(Couple *bride_parents, Couple *groom_parents){
  ...
  if(groom_parents->num_kids > 0){
    ...
    int num_kids_of_groom = rand() % MAX_NUM_KIDS + 1;
    add_some_kids(groom_parents, num_kids_of_groom);
    res->hubby = &groom_parents->kids[rand() % num_kids_of_groom];
  }
  ...
  if(bride_parents->num_kids > 0){
    ...
    int num_kids_of_bride = rand() % (MAX_NUM_KIDS + 1);
    add_some_kids(bride_parents, num_kids_of_bride);
    res->wife = &bride_parents->kids[rand() % num_kids_of_bride];
  }
  int num_of_res = rand() % (MAX_NUM_KIDS + 1);
  add_some_kids(res, num_of_res);
  res->num_kids = num_of_res;
